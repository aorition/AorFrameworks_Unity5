    ...
    public class T4MExtendsSC : Editor
    {
       // int layerMask = 1073741824;
        ...
        void OnSceneGUI()
        {
            ...
            if (oldState != State || !PlantObjPreview && T4MSC.T4MMenuToolbar == 4 ||
                T4MSC.T4MObjectPlant[T4MSC.T4MPlantSel] == null && T4MSC.T4MMenuToolbar == 4)
            {
        ...
        void Painter()
        {
            ...
            if (T4MSC.T4MPreview && T4MSC.T4MActived == "Activated" && T4MSC.T4MPreview.enabled == false ||
            ...
            else if (T4MSC.T4MPreview && T4MSC.T4MActived == "Deactivated" && T4MSC.T4MPreview.enabled == true ||
                     T4MSC.T4MPreview.enabled == true)
            {
                ...
                if (Physics.Raycast(terrain, out raycastHit, Mathf.Infinity))
                {
                    if (T4MSC.CurrentSelect.gameObject.GetComponent<T4MObjSC>().ConvertType != "UT")
                        T4MSC.T4MPreview.transform.localEulerAngles = new Vector3(90,
                            180 + T4MSC.CurrentSelect.localEulerAngles.y, 0);
                    else
                        T4MSC.T4MPreview.transform.localEulerAngles = new Vector3(90,
                            T4MSC.CurrentSelect.localEulerAngles.y, 0);
                    T4MSC.T4MPreview.transform.position = raycastHit.point; // +Vector3.up;
                        ...
                        T4MSC.PaintPrev != T4MSC.PaintHandle.Follow_Normal_WireCircle)
                    {
                        ...
                        Handles.DrawSolidDisc(raycastHit.point, raycastHit.normal,
                            T4MSC.T4MPreview.orthographicSize*0.9f);
                    }
                    else if (T4MSC.PaintPrev != T4MSC.PaintHandle.Classic &&
                             T4MSC.PaintPrev != T4MSC.PaintHandle.Hide_preview &&
                             T4MSC.PaintPrev != T4MSC.PaintHandle.Follow_Normal_Circle)
                    {
                        ...
                        (e.shift == false && e.alt == false && e.button == 0 && ToggleF == false))
                    {
                        ...
                        Color[] terrainBay = T4MSC.T4MUseOverrideColor
                            ? T4MSC.T4MOColorTex.GetPixels(x, y, width, height, 0)
                            : T4MSC.T4MMaskTex.GetPixels(x, y, width, height, 0);
                        ...
                        for (int i = 0; i < height; i++)
                        {
                            for (int j = 0; j < width; j++)
                            {
                                ...
                                float Stronger =
                                    T4MSC.T4MBrushAlpha[
                                        Mathf.Clamp((y + i) - (PuY - T4MSC.T4MBrushSizeInPourcent/2), 0,
                                            T4MSC.T4MBrushSizeInPourcent - 1)*T4MSC.T4MBrushSizeInPourcent +
                                        Mathf.Clamp((x + j) - (PuX - T4MSC.T4MBrushSizeInPourcent/2), 0,
                                            T4MSC.T4MBrushSizeInPourcent - 1)]*T4MSC.T4MStronger;

                                //aorition hacked
                                if (T4MSC.T4MUseOverrideColor)
                                {
                                    terrainBay[index] = Color.Lerp(terrainBay[index], T4MSC.T4MoverrideColor, Stronger);
                                }
                                else
                                {
                                    ...
                                    else
                                    {
                                        terrainBay[index] = Color.Lerp(terrainBay[index], T4MSC.T4MtargetColor, Stronger);
                                            //*0.3f);
                                        if (T4MSC.T4MMaskTex2)
                                            terrainBay2[index] = Color.Lerp(terrainBay2[index], T4MSC.T4MtargetColor2,
                                                Stronger); ///0.3f);
                                    }
                                }
                            }
                        ...
                        if (T4MSC.T4MUseOverrideColor)
                        {
                            ...
                            T4MSC.T4MOColorTex.SetPixels(x, y, width, height, terrainBay, 0);
                            T4MSC.T4MOColorTex.Apply();

                            UndoObj = new Texture2D[1];
                            UndoObj[0] = T4MSC.T4MOColorTex;
        ...
        void Planting()
        {
            ...
            if (T4MSC.T4MActived == "Activated")
            {
                ...
                if (PlantObjPreview &&
                    ...
                    new Vector3(T4MSC.T4MObjSize, T4MSC.T4MObjSize, T4MSC.T4MObjSize) && T4MSC.T4MSizeVar == 0 ||
                    PlantObjPreview && oldSize != T4MSC.T4MObjSize)
                {
                    PlantObjPreview.transform.localScale = new Vector3(T4MSC.T4MObjSize, T4MSC.T4MObjSize,
                        T4MSC.T4MObjSize);
                ...
                if (Physics.Raycast(terrain, out raycastHit, Mathf.Infinity))
                {
                    ...
                    if (CurrentObject)
                    {
                        if (T4MSC.T4MDistanceMin != T4MSC.T4MDistanceMax && T4MSC.T4MRandomSpa && RandomDistance == 0 ||
                            RandomDistance < T4MSC.T4MDistanceMin || RandomDistance > T4MSC.T4MDistanceMax)
                            RandomDist();
                        ...
                        if (!PlantObjPreview || currentObjPreview != CurrentObject)
                        {
                            ...
                            PlantObjPreview =
                                Instantiate(CurrentObject, raycastHit.point, Quaternion.identity) as GameObject;

                            T4MPlantRenderer = PlantObjPreview.GetComponentsInChildren<Renderer>();
                            for (int i = 0; i < T4MPlantRenderer.Length; i++)
                            {
                                if (T4MPlantRenderer[i].GetComponent<Renderer>() &&
                                    T4MPlantRenderer[i].GetComponent<Renderer>().sharedMaterial.HasProperty("_MainTex"))
                                {
                                    Texture Text =
                                        T4MPlantRenderer[i].GetComponent<Renderer>()
                                            .sharedMaterial.GetTexture("_MainTex");
                                    Material NewPMat = new Material(Shader.Find("Hidden/Preview"));
                                    T4MPlantRenderer[i].GetComponent<Renderer>().sharedMaterial = NewPMat;
                                    T4MPlantRenderer[i].GetComponent<Renderer>()
                                        .sharedMaterial.SetTexture("_MainTex", Text);
                                }
                            }
                        ...
                        if (e.shift == false && e.control == false)
                        {
                            ...
                            if (CheckPlacement != 0)
                            {
                                ...
                                {
                                    T4MPlantRenderer[i].GetComponent<Renderer>()
                                        .sharedMaterial.SetColor("_TintColor", new Color(1f, 0.5f, 0.5f, 0.071f));
                                }
                            }
                            else
                            {
                                ...
                                {
                                    T4MPlantRenderer[i].GetComponent<Renderer>()
                                        .sharedMaterial.SetColor("_TintColor", new Color(0.5f, 0.5f, 0.5f, 0.2f));
                                }
                            }
                            for (int i = 0; i < T4MPlantRenderer.Length; i++)
                            {
                        ...
                        if (T4MPlantObjGet != null && T4MPlantObjGet.Length > 0 && ToggleF == true)
                            for (int i = 0; i < T4MPlantObjGet.Length; i++)
                            {
                                ...
                                {
                                    if (
                                        Vector3.Distance(T4MPlantObjGet[i].transform.position,
                                            new Vector3(fix.x, T4MPlantObjGet[i].transform.position.y, fix.z)) <
                                        RandomDistance)
                                        CheckPlacement += 1;
                                }
                                else
                                {
                                    if (
                                        Vector3.Distance(T4MPlantObjGet[i].transform.position,
                                            new Vector3(fix.x, T4MPlantObjGet[i].transform.position.y, fix.z)) <
                                        T4MSC.T4MDistanceMin)
                                        CheckPlacement += 1;
                                }
                            }
                    }
                    if (e.type == EventType.mouseDown && e.alt == false && e.button == 0 && e.shift == false &&
                        e.control == false && PlantObjPreview ||
                        e.type == EventType.mouseDrag && e.alt == false && e.button == 0 && e.shift == false &&
                        e.control == false && PlantObjPreview)
                    {
                        if (CurrentObject)
                        {
                    ...
                    else if (e.type == EventType.mouseDown && e.alt == false && e.button == 0 && e.shift == true &&
                             e.control == false && PlantObjPreview ||
                             e.type == EventType.mouseDrag && e.alt == false && e.button == 0 && e.shift == true &&
                             e.control == false && PlantObjPreview)
                    {
                        if (ToggleF == false)
                        {
                    ...
                    else if (e.type == EventType.mouseDown && e.alt == false && e.button == 0 && e.shift == false &&
                             e.control == true && PlantObjPreview ||
                             e.type == EventType.mouseDrag && e.alt == false && e.button == 0 && e.shift == false &&
                             e.control == true && PlantObjPreview)
                    {
                        ...
                        {
                            T4MPlantObjGet =
                                GameObject.FindObjectsOfType(typeof (T4MPlantObjSC)) as T4MPlantObjSC[];
        ...
        void RandomRot()
        {
            RandomRotation = new Vector3(Random.Range(-T4MSC.T4MrandX*45, T4MSC.T4MrandX*45),
                Random.Range(-T4MSC.T4MrandY*180, T4MSC.T4MrandY*180),
                Random.Range(-T4MSC.T4MrandZ*45, T4MSC.T4MrandZ*45));
            if (PlantObjPreview)
            {
                PlantObjPreview.transform.rotation = Quaternion.FromToRotation(Vector3.up, Vector3.zero);
        ...
        void RandomSize()
        {
            ...
            if (PlantObjPreview)
            {
                ...
                PlantObjPreview.transform.transform.localScale = new Vector3(T4MSC.T4MObjSize + variation,
                    T4MSC.T4MObjSize + variation, T4MSC.T4MObjSize + variation);
            }
        }
            ...
            for (int i = 0; i < F.Length; i++)
            {
                GameObject test =
                    PrefabUtility.InstantiatePrefab(
                        (GameObject) AssetDatabase.LoadAssetAtPath(F[i], typeof (GameObject))) as GameObject;
