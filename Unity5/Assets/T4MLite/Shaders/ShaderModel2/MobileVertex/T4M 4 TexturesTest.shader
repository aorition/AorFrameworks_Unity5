Shader "T4MShaders/ShaderModel2/MobileVertex/T4M 4 Textures Test for Mobile" {
	Properties{
...
SubShader{
		...
		Pass
	{
		...
		{
			// sample the texture
			fixed4 splat_control = tex2D(_Control, i.uv0);
			...
			fixed4 col = (lay1 * splat_control.r + lay2 * splat_control.g + lay3 * splat_control.b + lay4 * splat_control.a)*i.diff;
...
#include "UnityCG.cginc"
		...
		v2f vert(a2v v)
		{
			...
			o.uv3 = TRANSFORM_TEX(v.uv, _Splat2);
			o.uv4 = TRANSFORM_TEX(v.uv, _Splat3);
			o.uv5 = mul(unity_LightmapMatrix, float4(v.uv1,0,1)).xy;
		...
		fixed4 frag(v2f i) : SV_Target
		{
			fixed4 lm = UNITY_SAMPLE_TEX2D(unity_Lightmap, i.uv5);
		fixed4 splat_control = tex2D(_Control, i.uv5);
		fixed4 lay1 = tex2D(_Splat0, i.uv1);
		fixed4 lay2 = tex2D(_Splat1, i.uv2);
		fixed4 lay3 = tex2D(_Splat2, i.uv3);
		fixed4 lay4 = tex2D(_Splat3, i.uv4);
		fixed4 col = lay1 * splat_control.r + lay2 * splat_control.g + lay3 * splat_control.b + lay4 * splat_control.a;
		col.rgb *= lm.rgb * 2;
