...
Shader "Custom/Light/Diffuse - Toon - Normal -line -UI" {
	Properties{
			...
			Pass {
				...
				#include "Assets/ObjectBaseShader.cginc"	
				 ...
				 //背光
						 ...
						 half backlight =saturate(dot(normal, normalize(lightDirection.xyz-half3(0.2,0,0.8))));
						 backlight = step(backlight, 0.1);
						 fixed3 backCol = fixed3(0.1, 0.1, 0.35)*backlight;
						 // return backlight;
						...
						//UI界面没辉光,用高光补强 
						half3 mainlight = col.rgb*lerp((ramp*_DirectionalLightDir.w*_DirectionalLightColor.rgb + ambientLight.rgb), ramp, _UILight) + i.lightColor.rgb ;
						
						
						final.rgb = mainlight*_Color.rgb+ backCol;
