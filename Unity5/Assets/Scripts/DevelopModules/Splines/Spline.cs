...
namespace Framework.module
{
    ...
    public class Spline : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField]
        public BezierControlPointMode[] _modes;
        ...
        {
            return new GameObject("Spline", typeof(Spline)).GetComponent<Spline>().init(RawPoints);
        }

        public Spline init(Vector3[] RawPoints)
        {
            _points = RawPoints;
            return this;
        }

        public void ImportData_StringList(string StringList)
        {
            string[] strs = StringList.Split(',');
            ...
            for (i = 0; i < length; i++)
            {
                ...
                newPoints[i].x = float.Parse(strs[i * 3]);
                newPoints[i].y = float.Parse(strs[i * 3 + 1]);
                newPoints[i].z = float.Parse(strs[i * 3 + 2]);
            }
        ...
        public float GetProgressAtSpeed(float currProgress, float velocity, float deltaTime, int direction = 1)
        {
            ...
            Vector3 v = GetVelocity(currProgress);

            nextT = currProgress + deltaTime * velocity / v.magnitude * step * direction;
        ...
        public float GetArcLengthParameter(float s, float epsilon = 0.0001f)
        {
            ...
            float length0 = s - _arcLengths[curveIndex]; // the arc length portion inside the _easeCurve
            float t0 = length0 / _curveLengths[curveIndex]; // the candidate t parameter inside the _easeCurve
        ...
        public Vector3[] GetSubdivisionB(float s0, float s1)
        {
            ...
            Debug.UiLog(d0 + " " + d1 + " " + p0 + " " + p3 + " " + pm + " " + k0 + " " + k1);
