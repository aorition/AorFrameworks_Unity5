...
//        protected List<object> _gridData;
        ...
        public void StartCreative(int dataCount, int maxRowNum, Action<ScrollListItem> customDrawListItem, string templateLoadPath = null,int jumpIndexX = 0, int jumpIndexY = 0) {

            if (dataCount <= 0 || maxRowNum <= 0) {
                Log.Error("GridPanel.StartCreative Error :  dataCount or maxRowNum is empty !");
                return;
            }
...
//        private List<ScrollListItem> _listItemCache;
        private ScrollListItem[] _listItemCache;
//        private List<ScrollListItem> _listItemTempCache;
        private ScrollListItem[] _listItemTempCache;
...
//            _newIDList = new List<GridPanelItemUV>();
        ...
        protected override void DrawUI() {
            ...
            List<ScrollListItem> tempListItems = new List<ScrollListItem>();
                ...
                for (u = _indexX; u < ulength; u++) {
                    ...
                    if (u < _gridData[v].Length) {
                        if (d < tempListItems.Count) {
                            ...
                            //new
                            go = GameObject.Instantiate(TemplateObject) as GameObject;
            ...
            _listItemTempCache = new ScrollListItem[_listItemCache.Length];
        ...
        private void updateListItem(int x, int y, ScrollListItem li, bool updateValue) {
        ...
        protected void setItemValue(ScrollListItem li, int x, int y) {
        ...
        private void DoScroll() {
                    ...
                    if (u < _rowMaxCount && v < _gridData.Length) {
                        if (u < _gridData[v].Length) {
                            ...
                            if (id < _gridDataCount) {
                                ScrollListItem li = getListItemFromCache(u, v);
                                if (li != null) {
            ...
            for (int i = 0; i < newIdNum; i++) {
                ...
                ScrollListItem li = findLastListItemFromCache();
                if (li != null) {
        ...
        private void ListItemTempCopy() {
            ...
            for (i = 0; i < len; i++) {
                ScrollListItem li = _listItemTempCache[i];
                _listItemCache[i] = li;
                _listItemTempCache[i] = null;
            }
        }

        private ScrollListItem findLastListItemFromCache() {
            for (int i = 0; i < _listItemCache.Length; i++) {
                if (_listItemCache[i] != null) {
                    ScrollListItem li = _listItemCache[i];
        ...
        private ScrollListItem getListItemFromCache(int u, int v) {
...
//            if (!_isDrawUIDone) return null;
        ...
        /// <summary>
        /// <接口> 自定义绘制ScrollListItem(ScrollList的子对象)的接口委托
        /// 
        /// 委托传入参数:
        /// ScrollListItem : ScrollListItem对象实例
        /// object : ScrollListItem对象的data数据;
        /// 
        /// </summary>
        public Action<ScrollListItem> CustomDrawListItem;

        /// <summary>
        /// <接口> 当ScrollListItem被点击时触发事件委托;
        /// 委托传入参数:
        /// GameObject : 被点击的ScrollListItem.gameObject对象
