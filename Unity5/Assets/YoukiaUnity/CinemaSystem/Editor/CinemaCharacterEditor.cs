...
public class CinemaCharacterEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(m_Object.FindProperty("ActorScale"));
        ...
        EditorGUILayout.PropertyField(m_Object.FindProperty("IgnoreActorBehavior"), new GUIContent("忽略角色动作", "勾选此项角色不在响应配置动作"));
        
        EditorGUILayout.PropertyField(m_Object.FindProperty("HideRole"), new GUIContent("隐藏当前角色", "勾选此项可隐藏当前角色"));
        ...
        if (GUILayout.Button(new GUIContent("创建角色用于动作编辑")))
        {
            if (EditorUtility.DisplayDialog("提示", "将创建一个关联角色用于动作编辑，此角色仅仅用于编辑中方便观察角色动作，请在动作编辑完成后手动删除该角色节点再保持预制体。", "明白了", "不明白"))
            {
                ...
                if (!isEx)
                {
                    ...
                    pre.transform.SetParent(_target.transform, false);
