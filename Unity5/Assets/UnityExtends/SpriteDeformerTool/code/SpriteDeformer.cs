...
namespace Medvedya.SpriteDeformerTools
{
    ...
    public abstract class SpriteDeformer : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void RecalculateSpriteInfo()
        {
            ...
            var pivotX = (bounds.center.x / bounds.extents.x / 2f - 0.5f) * spriteSizeInUnites.x;
            var pivotY = (bounds.center.y / bounds.extents.y / 2f - 0.5f) * spriteSizeInUnites.y;

            spritePivotInUnites = new Vector2(pivotX, pivotY);
#else           
        ...
        protected virtual void generateMeshData(bool new_tringulate, bool new_uv, bool new_points, bool new_color, bool new_normals)
        {
            ...
            {
                if (new_normals) mesh_normals = new Vector3[points.Count];
                if (new_color) mesh_colors = new Color[points.Count];
                if (new_points && mesh_vertecs == null && mesh_vertecs.Length != points.Count)
                    mesh_vertecs = new Vector3[points.Count];
                if (new_uv) mesh_uvs = new Vector2[points.Count];
                for (int i = 0; i < points.Count; i++)
                {
                    SpritePoint cp = points[i];
                    if (new_points)
                    {
        ...
        protected virtual void Update()
        {
            ...
            if (GetComponent<Renderer>().isVisible && (timeUpdate >= minimumTimeToUpdate || !Application.isPlaying))
            {
                timeUpdate = 0;
        ...
        /// <summary>
        /// Generate new data for the mesh and set this data to the mesh. It use dirty_* properties and will set all of these as false.
        /// </summary>
        ...
        /// <summary>
        /// Return sprite postion by SpritePoint.
        /// </summary>
        /// <param name="point"></param>
        /// <returns></returns>
        ...
        public void GenerateCollider()
        {
            ...
            for (int i = 0; i < borderPaths.Count; i++)
            {
                ...
                for (int j = 0; j < colliderPath.points.Length; j++)
                {
                    ...
                    cPoints[j] = SpritePositionToLocal(GetSpritePositionOfSpritePoint(currentPoint) + currentPoint.colliderOffset);
                }
