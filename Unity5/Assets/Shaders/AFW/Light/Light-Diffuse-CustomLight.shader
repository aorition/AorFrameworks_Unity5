...
//<readonly> 实时光照材质 <Fog><Clip><LightMap><Shadow><VertexAnim> <支持自定灯光>
//@@@DynamicShaderInfoEnd
Shader "AFW/Light/Light - Diffuse - CustomLight"
{
	Properties
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed3 shadow = lerp(_ShadowColor, col.rgb, atten);
					...
					// 光照方向
					#ifdef USECUSTOMLIGHT
						float3 worldLightDir = normalize(_DirectionalLightDir.xyz);
						col.rgb = _TintColor * _DirectionalLightColor.rgb * max(0, dot(worldNormal, worldLightDir));// 漫反射
					#else
				...
				#ifdef USECUSTOMLIGHT
					col.rgb = lerp(col.rgb, col.rgb * _DirectionalLightColor.rgb * _DirectionalLightColor.w, _SunLightThreshold);
