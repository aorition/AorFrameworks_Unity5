...
namespace ExoticUnity.GUI.AorUI.Components
{
    ...
    public class ScrollList : AorUIComponent
    {
        ...
        public void StartCreative(int dataCount, Action<ListItem> customDrawListItem, string templateLoadPath = null, uint startIndex = 0, int jumpIndex = 0)
        {
        ...
        protected override void DrawUI()
        {
            ...
            for (i = preIndex; i < length; i++)
            {
                ...
                {
                    //new
                    go = AorUIAssetLoader.Instantiate(_TemplateObject) as GameObject;
        ...
        private void updateListItem(int index, ListItem li, bool updateValue)
        {
        ...
        public void CheckItemSize(int index01, float item_size2, bool size)
        {
            ...
            for (i = (_index - _BufferNum < 0 ? 0 : _index - _BufferNum); i < length; i++)
            {
                if (i < _listData.Count)
                {
                    ...
                    ListItem lii = getListItemFromCache(i, false);
                    if (lii != null)
                    {
        ...
        /// 更新item尺寸
        /// </summary>
        private void UpdataItemSize(int index, ListItem li, int InputIndex, float size_y, bool add)
        {
        ...
        protected void setItemValue(ListItem li, int dataIndex)
        {
        ...
        private void DoScroll()
        {
            ...
            for (i = (_index - _BufferNum < 0 ? 0 : _index - _BufferNum); i < length; i++)
            {
                if (i < _listData.Count)
                {
                    ListItem li = getListItemFromCache(i);
                    if (li != null)
                    {
            ...
            if (_newIDList.Count > 0)
            {
                ...
                for (i = 0; i < _newIDList.Count; i++)
                {
                    ...
                    {
                        //新增
                        GameObject go = AorUIAssetLoader.Instantiate(_TemplateObject) as GameObject;
                        li = go.GetComponent<ListItem>();
                        if (li == null)
                        {
                            li = go.AddComponent<ListItem>();
        ...
        private ListItem getListItemFromCache(int index, bool delete = true)
        {
