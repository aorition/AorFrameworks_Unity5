...
public class RTFBrowser : MonoBehaviour
{
    ...
    public void SetOptions(List<string> filterList, bool isWhiteList = false, bool ignoreCase = true, bool endWithMatch = true)
    {
            ...
            for (i = 0; i < len; i++)
            {
                ...
                if (_filterList != null && _filterList.Count > 0)
                {
                    ...
                    bool pass = _ignoreCase
                        ? _filterList.Exists(
                            s =>
                                (_endWithMatch ? n1.ToLower().EndsWith(s.ToLower()) : n1.ToLower().Contains(s.ToLower())))
