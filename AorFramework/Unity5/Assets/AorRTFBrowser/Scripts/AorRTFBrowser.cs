...
/// 
/// </summary>
public class AorRTFBrowser : MonoBehaviour
{

    private const string _UIPrefabPath = "UIPrefab/AorRTFBrowser";

    public static void CreateBrowser(RectTransform parent, Action<AorRTFBrowser> finishCallback)
    {
        AorResourceLoader.LoadPrefab(_UIPrefabPath, g =>
        {
            g.transform.SetParent(parent, false);
            AorRTFBrowser view = g.AddComponent<AorRTFBrowser>();
            finishCallback(view);
        });
    }
    ...
    private void Awake()
    {
        ...
        _NGContent = _window.FindRectTransform("NGContent/ScrollView/Viewport/Content");
    ...
    public void Dispose()
    {
        AorResourceLoader.UnLoadPrefab(gameObject);
    }
    ...
    private void Start()
    {
        ...
        {
            _fdList = new List<string>(Application.dataPath.Split('/'));
        }else if (Application.platform == RuntimePlatform.WindowsPlayer)
        {
            _fdList = new List<string>(Application.persistentDataPath.Split('/'));
        }
    ...
    private void drawInnerContent()
    {
        ...
        for (i = 0; i < len; i++)
        {
            ...
            string fp = files[i].Replace('\\', '/');
            string sp = fp.Replace(p + "/", "");

            long bytes = 0;
            try
            {
                //Todo 这里可以使用一个更好的获取文件大小的方法
                byte[] fbytes = File.ReadAllBytes(fp);
            ...
            infoList.Add(new ARTFBFileInfo(sp, bytes, false));
        }
        ...
        for (i = 0; i < len; i++)
        {
            ...
            AorResourceLoader.LoadPrefab("UIPrefab/RTFBInfoItem", g =>
            {
    ...
    private void drawNGbtns()
    {
        ...
        AorResourceLoader.LoadPrefab("UIPrefab/RTBNGText", g =>
        {
    ...
    {
        AorResourceLoader.LoadPrefab("UIPrefab/RTBNGButton", g =>
        {
            g.transform.SetParent(_NGContent, false);
            //
