...
namespace YoukiaUnity.CinemaSystem
{
    ...
    public class FlyCamera : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                _rotate_camera(_camera, -_actionStatus[(int)FlyCameraAction.Left] * RotationSpeed, Vector3.up, _upVector);
                _actionStatus[(int)FlyCameraAction.Left] = 0;
            }
            if (_actionStatus[(int)FlyCameraAction.Right] != 0)
            {
                _rotate_camera(_camera, _actionStatus[(int)FlyCameraAction.Right] * RotationSpeed, Vector3.up, _upVector);
            ...
            if (_actionStatus[(int)FlyCameraAction.Up] != 0)
            {
                ...
                if (dotvy > -0.99f)
                {
                    _rotate_camera(_camera, -_actionStatus[(int)FlyCameraAction.Up] * RotationSpeed, axis, _upVector);
                }
            ...
            if (_actionStatus[(int)FlyCameraAction.Down] != 0)
            {
                ...
                if (dotvy < 0.99f)
                {
                    _rotate_camera(_camera, _actionStatus[(int)FlyCameraAction.Down] * RotationSpeed, axis, _upVector);
                }
            ...
            if (_actionStatus[(int)FlyCameraAction.Forward] != 0)
            {
                ...
                _move_camera(_camera, _actionStatus[(int)FlyCameraAction.Forward] * MoveSpeed * effectiveTPF, direction);
            }
            if (_actionStatus[(int)FlyCameraAction.Backword] != 0)
            {
                Vector3 direction = transform.forward;
                _move_camera(_camera, -_actionStatus[(int)FlyCameraAction.Backword] * MoveSpeed * effectiveTPF, direction);
            }
            if (_actionStatus[(int)FlyCameraAction.Leftword] != 0)
            {
                ...
                _move_camera(_camera, -_actionStatus[(int)FlyCameraAction.Leftword] * MoveSpeed * effectiveTPF, left);
            }
            if (_actionStatus[(int)FlyCameraAction.Rightword] != 0)
            {
                ...
                Vector3 up = transform.up;
                Vector3 left = Vector3.Cross(up, direction);
                _move_camera(_camera, _actionStatus[(int)FlyCameraAction.Rightword] * MoveSpeed * effectiveTPF, left);
            }
            if (_actionStatus[(int)FlyCameraAction.Rise] != 0)
            {
                Vector3 up = Vector3.up;
                _move_camera(_camera, _actionStatus[(int)FlyCameraAction.Rise] * MoveSpeed * effectiveTPF, up);
            }
            if (_actionStatus[(int)FlyCameraAction.Lower] != 0)
            {
                Vector3 up = Vector3.up;
                _move_camera(_camera, -_actionStatus[(int)FlyCameraAction.Lower] * MoveSpeed * effectiveTPF, up);
            }
            if (_actionStatus[(int)FlyCameraAction.ZoomIn] != 0)
            {
                _zoom_camera(_camera, -_actionStatus[(int)FlyCameraAction.ZoomIn] * ZoomSpeed);
                _actionStatus[(int)FlyCameraAction.ZoomIn] = 0;
            }
            if (_actionStatus[(int)FlyCameraAction.ZoomOut] != 0)
            {
                _zoom_camera(_camera, _actionStatus[(int)FlyCameraAction.ZoomOut] * ZoomSpeed);
                _actionStatus[(int)FlyCameraAction.ZoomOut] = 0;
            }
        }

        static void _rotate_camera(Camera cam, float value, Vector3 axis, Vector3 up)
        {
            Quaternion quatSrc = cam.transform.rotation;
            ...
            cam.transform.up = fixedUp;
            cam.transform.forward = direction;
        }

        static void _move_camera(Camera cam, float value, Vector3 axis)
        {
            Vector3 vec;
            Vector3 vecSrc = cam.transform.position;
            vec.x = axis.x * value;
            vec.y = axis.y * value;
            vec.z = axis.z * value;
            vec += vecSrc;
        ...
        static void _zoom_camera(Camera cam, float value)
        {
            ...
            {
                cam.fieldOfView = cam.fieldOfView * zoom;
            }
        }
