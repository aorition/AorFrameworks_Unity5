...
namespace PigeonCoopToolkit.Effects.Trails
{
	public abstract class TrailRenderer_Base : MonoBehaviour
	{
		...
		{
			float delta = IsPause ? 0 : Time.deltaTime;
			if (_hasRenderer)
			{
				_hasRenderer = false;
				...
				if (_toClean.Count > 0)
				{
					...
					int _count = _toClean.Count;
					for (int i=0; i < _count; ++i)
					{
						if (Application.isEditor)
							DestroyImmediate (_toClean[i], true);
						else
							Destroy (_toClean[i]);
					}
				}
			...
			{
				UpdatePoints (_activeTrail, delta);
				UpdateTrail (_activeTrail, delta);
			...
			for (int i = _fadingTrails.Count - 1; i >= 0; i--)
			{
				...
				UpdatePoints (_fadingTrails[i], delta);
				UpdateTrail (_fadingTrails[i], delta);
