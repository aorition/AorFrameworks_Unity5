...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        private void Awake()
        {
            ...
            if (this.logMessages)
                Log.Debug(string.Format("SpawnPool {0}: Initializing..", this.poolName));
            ...
            for (int i = 0; i < this._perPrefabPoolOptions.Count; i++)
            {
                ...
                {
//                    Log.warning(string.Format("Initialization Warning: Pool '{0}' " +
...
//                               this.poolName));
        ...
        {
            if (this.logMessages)
                Log.Debug(string.Format("SpawnPool {0}: Destroying...", this.poolName));
        ...
        public void CreatePrefabPool(PrefabPool prefabPool)
        {
            ...
            {
                if (this.logMessages)
                    Log.Debug(string.Format("SpawnPool {0}: Preloading {1} {2}",
                                               this.poolName,
...
#region Pool Functionality
        ...
        {
            if (this.logMessages)
                Log.Debug(string.Format("SpawnPool {0} ({1}): Despawning '{2}'",
        ...
        {
            if (this.logMessages)
                Log.Debug(string.Format("SpawnPool {0} ({1}): CULLING TRIGGERED! " +
            ...
            while (this.totalCount > this.cullAbove)
            {
                ...
                for (int i = 0; i < this.cullMaxPerPass; i++)
                {
                    ...
                    if (this._despawned.Count > 0)
                    {
                        ...
                        if (this.logMessages)
                            Log.Debug(string.Format("SpawnPool {0} ({1}): " +
            ...
            if (this.logMessages)
                Log.Debug(string.Format("SpawnPool {0} ({1}): CULLING FINISHED! Stopping",
                                        this.spawnPool.poolName,
                                        this.prefab.name));
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
            ...
            else
            {
                ...
                if (this.logMessages)
                    Log.Debug(string.Format("SpawnPool {0} ({1}): respawning '{2}'.",
                                            this.spawnPool.poolName,
                                            this.prefab.name,
                                            inst.name));
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            if (this.logMessages)
                Log.Debug(string.Format("SpawnPool {0} ({1}): Spawned new instance '{2}'.",
                                        this.spawnPool.poolName,
                                        this.prefab.name,
                                        inst.name));
...
//                );
            ...
            {
//                Log.warning(string.Format("SpawnPool {0} ({1}): " +
