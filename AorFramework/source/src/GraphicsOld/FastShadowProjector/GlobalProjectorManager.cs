...
namespace YoukiaUnity.Graphics.FastShadowProjector
{
    ...
    public class GlobalProjectorManager : MonoBehaviour
    {
      ...
      //  [NonSerialized]
        public Material _PreShadowMaterialAlpha;
      //  [NonSerialized]
        public Material _PreShadowMaterial;
        ...
        public static GlobalProjectorManager CreateInstance()
        {

            GlobalProjectorManager _Instance = new GameObject("_FSPGlobalProjectorManager").AddComponent<GlobalProjectorManager>();
        ...
        public void LoadConfig()
        {
            ...
            //            ConfigPlus.ZClipFar = 130;
            //            ConfigPlus.ProjectionAngles = new Vector3(90, 0, 0);
            //            ConfigPlus.ShadowResolution = GlobalProjectorManager.ShadowResolutions.Low_256;
            //            ConfigPlus.ShadowType = GlobalProjectorManager.ShadowType.ModelProjection;
        ...
        public void Initialize()
        {
            ...
            _ProjectorMaterialShadowAlpha = new Material(Shader.Find("Fast Shadow Projector/MultiplyAlpha"));
            _ProjectorMaterialLight = new Material(Shader.Find("Fast Shadow Projector/Add"));

            _PreShadowMaterialAlpha = new Material(Shader.Find("Fast Shadow Projector/PreModelAlpha"));
            _PreShadowMaterial = new Material(Shader.Find("Fast Shadow Projector/PreModel"));
        ...
        {
            if (_LightProjectors != null && _LightProjectors.Count > 0)
            {
                CreateProjectorEyeTexture();
            }
            else
        ...
        void CreateProjectorEyeTexture(bool shadow, bool light)
        {
            if (shadow)
            {
                ...
                if (_ProjectorCamera != null && _ShadowResolutions != null)
                {
        ...
        protected Vector3[] getFrustumPoints(Camera cam, float zFar, Vector3[] result)
        {
            if (cam == null)
                return result;

            result = result ?? new Vector3[8]; 
        ...
        protected Vector3[] getFrustumPoints(Camera cam, Vector3[] result)
        {
            if (cam == null)
                return result;
        ...
        {
            Vector3[] shadowPoints = new Vector3[8];
            ...
            shadowPoints = GetBoundsPoints(frustumAABB);
            ...
            for (int i = 0; i < projectors.Count; i++)
            {
                ...
                bounds = Transform(tempBounds, viewMat);
                ...
                if (_GlobalShadowType == projector.Type && frustumAABB.Intersects(bounds))
                {
                    if (projectorCount == 0)
                    {
                        casterAABB = bounds;
...
//            shadowPoints = getFrustumPoints(shadowCam, shadowPoints);
//            if (lines == null)
//            {
//                lines = new GameObject[12];
//
//                for (int i = 0; i < 12; i++)
//                {
//                    lines[i] = new GameObject("Line_" + i);
...
//            }
//
//            lines[0].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[0], shadowPoints[1] });
//            lines[1].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[1], shadowPoints[2] });
//            lines[2].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[2], shadowPoints[3] });
//            lines[3].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[3], shadowPoints[0] });
//            lines[4].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[4], shadowPoints[5] });
//            lines[5].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[5], shadowPoints[6] });
//            lines[6].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[6], shadowPoints[7] });
//            lines[7].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[7], shadowPoints[4] });
//            lines[8].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[0], shadowPoints[4] });
//            lines[9].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[1], shadowPoints[5] });
//            lines[10].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[2], shadowPoints[6] });
//            lines[11].GetComponent<LineRenderer>().SetPositions(new[] { shadowPoints[3], shadowPoints[7] });

        }

        public static Vector3[] GetBoundsPoints(Bounds b)
        {
            Vector3[] r = new Vector3[8];
            r[0] = b.center + new Vector3(b.extents.x, -b.extents.y, -b.extents.z);
            r[1] = b.center + new Vector3(b.extents.x, -b.extents.y, b.extents.z);
            r[2] = b.center + new Vector3(b.extents.x, b.extents.y, b.extents.z);
            r[3] = b.center + new Vector3(b.extents.x, b.extents.y, -b.extents.z);
            r[4] = b.center + new Vector3(-b.extents.x, -b.extents.y, -b.extents.z);
            r[5] = b.center + new Vector3(-b.extents.x, -b.extents.y, b.extents.z);
            r[6] = b.center + new Vector3(-b.extents.x, b.extents.y, b.extents.z);
            r[7] = b.center + new Vector3(-b.extents.x, b.extents.y, -b.extents.z);
...
//        GameObject[] lines;
        ...
        void Update()
        {
            ...
            if (_ShadowReceivers == null) return;
        ...
        void RenderProjectors(Camera targetCamera, List<IProjector> projectors, Material material)
        {
            ...
            if (projectors != null && projectors.Count > 0 && _ShadowReceivers != null && _ShadowReceivers.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (useMBP)
                    {
                        ...
                        for (int n = 0; n < _ShadowReceivers[i].GetMesh().subMeshCount; n++)
                        {
                            ...
                            UnityEngine.Graphics.DrawMesh(_ShadowReceivers[i].GetMesh(), _ModelMatrix, material, LayerMask.NameToLayer("Default"), GraphicsManager.GetInstance().MainCamera, n, _MBP);
                        }
                    }
                    else
                    {
                        ...
                        UnityEngine.Graphics.DrawMesh(_ShadowReceivers[i].GetMesh(), _ModelMatrix, material, LayerMask.NameToLayer("Default"), GraphicsManager.GetInstance().MainCamera);
                    }
                }
            }
        }
        ...
        void CheckTriggers(bool shadow)
        {
            ...
            {
                //                if (Physics.Raycast(new Ray(_ShadowTriggers[n].transform.position, YKApplication.Instance.GetManager<GraphicsManager>().ShadowMgr.GlobalProjectionDir), out raycastHit))
                if (Physics.Raycast(new Ray(_ShadowTriggers[n].transform.position, _graphicsManager.ShadowMgr.GlobalProjectionDir), out raycastHit))
                {
