...
namespace Framework.Graphic
{
   ...
   // [AddComponentMenu("Cinemachine/CinemachineTargetGroup")]
    [ExecuteInEditMode]
    public class VCETargetGroup : MonoBehaviour
    {
        ...
        public Bounds BoundingBox
        {
            get
            {
                ...
                if (averageWeight > FloatExtends.Epsilon)
                {
                    for (int i = 0; i < m_Targets.Length; ++i)
                    {
                        if (m_Targets[i].target != null)
                        {
                            ...
                            if (w < averageWeight - FloatExtends.Epsilon)
                                w = w / averageWeight;
                            else
                                w = 1;
        ...
        public bool IsEmpty 
        {
            ...
            {
                for (int i = 0; i < m_Targets.Length; ++i)
                    if (m_Targets[i].target != null && m_Targets[i].weight > FloatExtends.Epsilon)
                        return false;
                return true;
            }
        }
        ...
        Vector3 CalculateAveragePosition(out float averageWeight)
        {
            ...
            if (weight > FloatExtends.Epsilon)
                pos /= weight;
            if (numTargets == 0)
            {
                averageWeight = 0;
