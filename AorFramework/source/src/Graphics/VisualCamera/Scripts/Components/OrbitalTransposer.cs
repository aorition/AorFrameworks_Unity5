...
namespace Framework.Graphic
{
    ...
    public class OrbitalTransposer : Transposer
    {
        /// <summary>
        ...
        internal delegate float UpdateHeadingDelegate(
            OrbitalTransposer orbital, float deltaTime, Vector3 up);
...
//            if (deltaTime >= 0 || CinemachineCore.Instance.IsLive(VirtualCamera))
            ...
            else
            {
                    ...
                    && (Time.time > (mLastHeadingAxisInputTime + m_RecenterToTargetHeading.m_RecenterWaitTime)))
                {
                    ...
                    else
                    {
                        ...
                        if (absHeadingError < FloatExtends.Epsilon)
                        {
                            axis.Value = targetHeading;
                            mHeadingRecenteringVelocity = 0;
                        }
        ...
        public override void MutateCameraState(float deltaTime)
        {
            ...
            InitPrevFrameStateInfo(deltaTime);
            ...
            float heading = HeadingUpdater(this, deltaTime, extension.ReferenceUp);
            ...
            if (IsValid)
            {
                ...
                Quaternion headingRot = Quaternion.AngleAxis(heading, extension.ReferenceUp);

                // Track the target, with damping
                Vector3 offset = EffectiveOffset;
                Vector3 pos;
                Quaternion orient;
                TrackTarget(deltaTime, extension.ReferenceUp, headingRot * offset, out pos, out orient);

                // Place the camera
                extension.ReferenceUp = orient * Vector3.up;
                if (deltaTime >= 0)
                {
                    Vector3 bypass = (headingRot * offset) - mHeadingPrevFrame * mOffsetPrevFrame;
                    bypass = orient * bypass;
                    extension.PositionDampingBypass = bypass;
                }
                orient = orient * headingRot;
                extension.RawPosition = pos + orient * offset;
            ...
            float currentHeading, Quaternion targetOrientation, float deltaTime)
        {
                ...
                && mTargetRigidBody == null)
            {
                Debug.Log(string.Format(
                        ...
                        extension.Target.name, FollowTarget));
        ...
        class HeadingTracker
        {
            ...
            void PopBottom()
            {
                if (mCount > 0)
                {
                    ...
                    if (mWeightSum <= FloatExtends.Epsilon || mCount == 0)
                        ClearHistory();
                }
            }
            ...
            public void DecayHistory()
            {
                ...
                mWeightSum *= decay;
                mWeightTime = time;
                if (mWeightSum < FloatExtends.Epsilon)
                    ClearHistory();
                else
