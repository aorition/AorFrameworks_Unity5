...
namespace Framework.Graphic
{
    ...
    public class FramingTransposer : VisualCameraComponentBase, IVisualCameraBodyComponent
    {
        ...
        public override bool IsValid { get { return this != null && enabled && FollowTarget != null && LookAtTarget == null; } }
        ...
        public override void MutateCameraState(float deltaTime)
        {
            ...
            {
                m_Predictor.Reset();
                m_PreviousCameraPosition = extension.RawPosition
                    + (extension.RawOrientation * Vector3.back) * m_CameraDistance;
            }
            if (!IsValid)
                return;
            ...
            extension.ReferenceLookAt = FollowTarget.position;
            m_Predictor.Smoothing = m_LookaheadSmoothing;
            m_Predictor.AddPosition(extension.ReferenceLookAt);
            TrackedPoint = (m_LookaheadTime > 0)
                ? m_Predictor.PredictPosition(m_LookaheadTime) : extension.ReferenceLookAt;

            // Work in camera-local space
            Quaternion localToWorld = extension.RawOrientation;
            ...
            VCETargetGroup group = TargetGroup;
            if (group != null && m_GroupFramingMode != FramingMode.None)
                cameraOffset.z += AdjustCameraDepthAndLensForGroupFraming(
                    group, targetPos.z - cameraOffset.z, deltaTime);

            // Move along the XY plane
            targetPos.z -= cameraOffset.z;
            float screenSize = GraphicsManager.instance.MainCamera.orthographic
                ? GraphicsManager.instance.MainCamera.orthographicSize
                : Mathf.Tan(0.5f * GraphicsManager.instance.MainCamera.fieldOfView * Mathf.Deg2Rad) * targetPos.z;
            Rect softGuideOrtho = ScreenToOrtho(SoftGuideRect, screenSize, GraphicsManager.instance.MainCamera.aspect);
            if (deltaTime < 0)
            {
            ...
            else
            {
                ...
                {
                    Rect hardGuideOrtho = ScreenToOrtho(HardGuideRect, screenSize, GraphicsManager.instance.MainCamera.aspect);
            ...
            extension.RawPosition = m_PreviousCameraPosition = localToWorld * (cameraPos + cameraOffset);
        ...
        public VCETargetGroup TargetGroup
        {
            get
            {
                Transform follow = FollowTarget;
                if (follow != null)
                    return follow.GetComponent<VCETargetGroup>();
                return null;
            }
        }

        float AdjustCameraDepthAndLensForGroupFraming(
            VCETargetGroup group, float targetZ,
            float deltaTime)
        {
            ...
            Vector3 fwd = extension.RawOrientation * Vector3.forward;
            m_lastBoundsMatrix = Matrix4x4.TRS(
                    bounds.center - (fwd * bounds.extents.magnitude),
                    extension.RawOrientation, Vector3.one);
            ...
            // Move the camera
            if (!GraphicsManager.instance.MainCamera.orthographic && m_AdjustmentMode != AdjustmentMode.ZoomOnly)
            {
                // What distance would be needed to get the target height, at the current FOV
                float desiredDistance
                    = targetHeight / (2f * Mathf.Tan(GraphicsManager.instance.MainCamera.fieldOfView * Mathf.Deg2Rad / 2f));
            ...
            // Apply zoom
            if (GraphicsManager.instance.MainCamera.orthographic || m_AdjustmentMode != AdjustmentMode.DollyOnly)
            {
                ...
                GraphicsManager.instance.MainCamera.fieldOfView = Mathf.Clamp(currentFOV, m_MinimumFOV, m_MaximumFOV);
                GraphicsManager.instance.MainCamera.orthographicSize = Mathf.Clamp(targetHeight / 2, m_MinimumOrthoSize, m_MaximumOrthoSize);
            }
            return -cameraOffset;
        }
        ...
        float GetTargetHeight(Bounds b)
        {
            ...
            switch (m_GroupFramingMode)
            {
                case FramingMode.Horizontal:
                    return b.size.x / (framingSize * GraphicsManager.instance.MainCamera.aspect);
                case FramingMode.Vertical:
                    return b.size.y / framingSize;
                default:
                case FramingMode.HorizontalAndVertical:
                    return Mathf.Max(
                        b.size.x / (framingSize * GraphicsManager.instance.MainCamera.aspect),
