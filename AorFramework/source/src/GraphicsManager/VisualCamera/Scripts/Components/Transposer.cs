...
namespace Framework.Graphic
{
    ...
    public class Transposer : VisualCameraComponentBase, IVisualCameraBodyComponent
    {
        /// <summary>
        ...
        public override void MutateCameraState(float deltaTime)
        {
            ...
            if (IsValid)
            {
                ...
                TrackTarget(deltaTime, extension.ReferenceUp, offset, out pos, out orient);
        ...
        protected void InitPrevFrameStateInfo(float deltaTime)
        {
            ...
            if (deltaTime < 0)
            {
                m_PreviousTargetPosition = extension.Target.transform.position;
                m_PreviousReferenceOrientation = GetReferenceOrientation(extension.ReferenceUp);
            }
        }
            ...
            out Vector3 outTargetPosition, out Quaternion outTargetOrient)
        {
            ...
            if (deltaTime >= 0)
            {
                ...
                if (desiredCameraOffset.AlmostZero())
                    dampingSpace = extension.RawOrientation;
                else
        ...
        public Quaternion GetReferenceOrientation(Vector3 worldUp)
        {
            if (FollowTarget != null)
            {
                ...
                switch (m_BindingMode)
                {
                    ...
                    case BindingMode.SimpleFollowWithWorldUp:
                        {
                            Vector3 dir = FollowTarget.position - extension.RawPosition;
                            if (dir.AlmostZero())
                                break;
