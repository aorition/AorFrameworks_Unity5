...
namespace Framework.Graphic
{
    ...
    [SaveDuringPlay]
    public class Composer : VisualCameraComponentBase, IVisualCameraAimComponent
    {
...
//        }
        ...
        public override void MutateCameraState(float deltaTime)
        {
            ...
            extension.ReferenceLookAt = GetLookAtPointAndSetTrackedPoint(extension.ReferenceLookAt);


            float targetDistance = (TrackedPoint - extension.CorrectedPosition).magnitude;
            if (targetDistance < Epsilon)
            {
                if (deltaTime >= 0)
                    extension.RawOrientation = m_CameraOrientationPrevFrame;
                return;  // navel-gazing, get outa here
            }

            //UnityEngine.Profiling.Profiler.BeginSample("CinemachineComposer.MutateCameraState");
            float fov, fovH;
            if (extension.Lens.orthographic)
            {
                // Calculate effective fov - fake it for ortho based on target distance
                fov = Mathf.Rad2Deg * 2 * Mathf.Atan(extension.Lens.orthographicSize / targetDistance);
                fovH = Mathf.Rad2Deg * 2 * Mathf.Atan(
                    extension.Lens.aspect * extension.Lens.orthographicSize / targetDistance);
            }
            else
            {
                fov = extension.Lens.fieldOfView;
                double radHFOV = 2 * Math.Atan(Math.Tan(fov * Mathf.Deg2Rad / 2) * extension.Lens.aspect);
                fovH = (float)(Mathf.Rad2Deg * radHFOV);
            }

            Quaternion rigOrientation = extension.RawOrientation;
            Rect softGuideFOV = ScreenToFOV(SoftGuideRect, fov, fovH, extension.Lens.aspect);
            if (deltaTime < 0)
            {
                ...
                RotateToScreenBounds(rect, ref rigOrientation, fov, fovH, -1);
            }
            else
            {
                // Start with previous frame's orientation (but with current up)
                Vector3 dir = m_LookAtPrevFrame - (m_CameraPosPrevFrame + extension.PositionDampingBypass);
                if (dir.AlmostZero())
                    rigOrientation = Quaternion.LookRotation(
                        m_CameraOrientationPrevFrame * Vector3.forward, extension.ReferenceUp);
                else
                {
                    rigOrientation = Quaternion.LookRotation(dir, extension.ReferenceUp);
                    rigOrientation = rigOrientation.ApplyCameraRotation(
                        -m_ScreenOffsetPrevFrame, extension.ReferenceUp);
                }

                // First force the previous rotation into the hard bounds, no damping, 
                // then Now move it through the soft zone, with damping
                Rect hardGuideFOV = ScreenToFOV(HardGuideRect, fov, fovH, extension.Lens.aspect);
                if (!RotateToScreenBounds(hardGuideFOV, ref rigOrientation, fov, fovH, -1))
                    RotateToScreenBounds(softGuideFOV, ref rigOrientation, fov, fovH, deltaTime);
            }
            m_CameraPosPrevFrame = extension.CorrectedPosition;
...
//            m_CameraOrientationPrevFrame = UnityQuaternionExtensions.Normalized(rigOrientation);
            ...
            m_ScreenOffsetPrevFrame = m_CameraOrientationPrevFrame.GetCameraRotationToTarget(
                m_LookAtPrevFrame - extension.CorrectedPosition, extension.ReferenceUp);
        ...
        /// </summary>
        private bool RotateToScreenBounds(Rect screenRect,
            ref Quaternion rigOrientation, float fov, float fovH, float deltaTime)
        {
            Vector3 targetDir = TrackedPoint - extension.CorrectedPosition;
            Vector2 rotToRect = rigOrientation.GetCameraRotationToTarget(targetDir, extension.ReferenceUp);

            // Bring it to the edge of screenRect, if outside.  Leave it alone if inside.
            ClampVerticalBounds(ref screenRect, targetDir, extension.ReferenceUp, fov);
            ...
            // Rotate
            rigOrientation = rigOrientation.ApplyCameraRotation(rotToRect, extension.ReferenceUp);
#if false
