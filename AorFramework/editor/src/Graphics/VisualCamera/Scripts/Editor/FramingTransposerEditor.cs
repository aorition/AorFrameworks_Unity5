...
{
    [CustomEditor(typeof(FramingTransposer))]
    internal class FramingTransposerEditor : BaseEditor<FramingTransposer>
    {
        ...
        protected override List<string> GetExcludedPropertiesInInspector()
        {
            ...
            VCETargetGroup group = Target.TargetGroup;
            if (group == null || Target.m_GroupFramingMode == FramingTransposer.FramingMode.None)
            {
            ...
            else 
            {
                ...
                else 
                {
                    ...
                    {
                    case FramingTransposer.AdjustmentMode.DollyOnly:
                        excluded.Add(FieldPath(x => x.m_MinimumFOV));
                        excluded.Add(FieldPath(x => x.m_MaximumFOV));
                        break;
                    case FramingTransposer.AdjustmentMode.ZoomOnly:
                        excluded.Add(FieldPath(x => x.m_MaxDollyIn));
                        excluded.Add(FieldPath(x => x.m_MaxDollyOut));
                        excluded.Add(FieldPath(x => x.m_MinimumDistance));
                        excluded.Add(FieldPath(x => x.m_MaximumDistance));
...
//            if (!Target.IsValid || !GMVSettings.GMVCoreSettings.ShowInGameGuides)
//                return;
...
//        }
        ...
        [DrawGizmo(GizmoType.Active | GizmoType.InSelectionHierarchy, typeof(FramingTransposer))]
        private static void DrawGroupComposerGizmos(FramingTransposer target, GizmoType selectionType)
        {
