                ...
                if (checker)
                {
                    ...
                    EditorPlusMethods.NextEditorApplicationUpdateDo(() =>
                    {
                        try
                        {
                            ...
                            {
                                GameObject.DestroyImmediate(checker);
                            }
                        }
                        catch (Exception ex)
                        {
...
//                checker.mainTexture = null;
        ...
        private void __Draw_meshExportUI()
        {
            ...
            GUILayout.BeginHorizontal();
            RecalculateNormals = EditorGUILayout.Toggle(new GUIContent("Recalculate Normals", "重新计算Normal数据"), RecalculateNormals);
            RecalculateTangents = EditorGUILayout.Toggle(new GUIContent("Recalculate Tangents", "重新计算Tangent数据"), RecalculateTangents);
            GUILayout.EndHorizontal();
            ...
            if (GUILayout.Button(new GUIContent(buttonLabel, buttonTip), GUILayout.Height(32)))
            {
                ...
                if (JOBExport)
                {
                    MeshExportUtil.ExprotMeshWithWorldTranToOBJ(target as MeshFilter, savePath, ReMeshName, RecalculateNormals, RecalculateTangents);
                }
                else
                {
                    MeshExportUtil.ExportMeshWithWorldTran(target as MeshFilter, savePath, CreatePrefab, ReMeshName, RecalculateNormals, RecalculateTangents);
                }
